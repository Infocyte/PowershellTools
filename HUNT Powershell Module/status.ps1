...
function Get-ICJobs ([Switch]$Active){
	...
	if ($Active) {
		Write-Host "Getting Active Jobs from Infocyte HUNT: $HuntServer"
		$filter['where'] = @{ state = "Active" }
	} else {
		Write-Host "Getting All Jobs from Infocyte HUNT: $HuntServer"
	}
...
function Get-ICUserTasks ([Switch]$Active, [Switch]$IncludeArchived){
	if ($IncludeArchived) {
		Write-Host "Getting All User Tasks from Infocyte HUNT: $HuntServer"
	...
	if ($Active) {
		Write-Host "Getting Active Tasks from Infocyte HUNT: $HuntServer"
		$filter['where'] = @{ status = "Active" }
	} else {
		Write-Host "Getting All Tasks from Infocyte HUNT: $HuntServer"
	}
	_ICGetMethod -url $url -filter $filter
}
...
function Get-ICLastScanid {
	Write-Host "Getting last scanId from Infocyte HUNT: $HuntServerAddress"
